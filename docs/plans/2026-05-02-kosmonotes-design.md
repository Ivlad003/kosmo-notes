# KosmoNotes — Voice-First AI Capture for macOS
...
**Working title:** KosmoNotes (final name TBD — see §16)
...
| Tool | What it does well | Where KosmoNotes differs |
|---|---|---|
...
4. **No bots in meetings** — system audio capture, never a "KosmoNotes has joined the meeting" notification.
...
```
┌──────────────────────────────────────────────────────────────────────┐
│  KosmoNotes — single .app, ~10 MB target                            │
│                                                                      │
│  ┌────────────────────┐  ┌──────────────────┐  ┌──────────────────┐ │
...
| **D15** | **v1.0 transcription is batch, not streaming. Decision applied 2026-05-03 after audit `docs/audits/2026-05-03-architecture-code-spec-audit.md` §"streaming-vs-batch transcription drift".** `RecorderState.stop()` submits `audio.m4a` to the user-selected batch provider (Whisper / AssemblyAI / Deepgram batch) once the recording finishes. The streaming infrastructure (`TranscriptionKit/DeepgramProvider.openResilientSession`, `ReconnectingSession` with 5-s ring buffer + exponential backoff) is **kept in tree** and unit-tested, but **not wired into the recorder UI**. Reasons: (a) batch is end-to-end working and shipped; (b) streaming requires a PCM tee from `CaptureSession` plus live-transcript UI surface, neither of which exists in v1.0; (c) for a single-user product without live captions in v1.0, batch latency (≤5 s after stop on 5-min recording) is good enough. AC-7 ("Deepgram streaming … within 5 s of stop") is reinterpreted as a batch-completion budget for v1.0. | v1.0 ships, owner wants live captions during recording, and a CaptureSession PCM tee is on the v1.1 roadmap. At that point: wire `openResilientSession` into `RecorderState`, add a live-transcript subview, add stop-while-streaming race tests. |
| **D16** | **WhisperKit reinstated as opt-in fifth transcription provider (2026-05-04). Owner reversed the cloud-only invariant after using v0.0.2.** `Sources/TranscriptionKit/WhisperKitProvider.swift` is a `BatchTranscriptionProvider`-conforming actor wrapping `argmaxinc/WhisperKit` (resolved 0.18.0). Models are downloaded on demand via `WhisperKitModelManager` into `~/Library/Application Support/KosmoNotes/whisperkit/<variant>/`. Settings → Transcription gets a "Local — WhisperKit" section with picker + Download/Delete buttons + progress bar, exposing the full HuggingFace catalogue (tiny → large-v3-turbo + distil variants). The active provider stays at whatever cloud option the user last chose — **WhisperKit is NOT auto-selected on download**. `RecorderState.start` pre-flights the WhisperKit case (variant picked + downloaded) so users can't start a recording that will fail because no model is on disk. Bundle stays under 15 MB (only the framework ships, weights are runtime downloads). `WhisperKit.recommendedModels()` is surfaced as a hint near the picker. | Argmax pivots WhisperKit to a license that conflicts with personal-binary distribution (currently MIT). Apple ships a comparable on-device transcription API at parity with Whisper (could simplify by removing the third-party dep). User opts to make WhisperKit the default — revisit "default cloud" stance. Parakeet (NVIDIA TDT via FluidInference/swift-parakeet-mlx) becomes a clearly-better local option — add it as a sixth provider. |

### Brainstorm-recorded — do not relitigate

(Equivalent to Jarvis Studio §12.) The pivot from Jarvis Studio → KosmoNotes happened on 2026-05-02 after four review rounds, three stack pivots (Tauri → iced → Swift), and a working v0.1.1. The decisions above are post-pivot. Reverting to a Rust core or webview frontend is **out of scope** for v1 and would invalidate the entire spec. Screen recording was originally deferred but was re-admitted as an opt-in feature (D14); this does not constitute a full product pivot.
...
5. **Final product name.** Working title: KosmoNotes. Other options surfaced in conversation: "Whisper Pad" (conflicts with OpenAI Whisper), "Voicelet", "Pulse", "Threadnote", "Echo" (generic). Pick before first beta share.
