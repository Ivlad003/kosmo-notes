# KosmoNotes v1.0 — Implementation Plan (scope-reduced after Critic review)
...
**Date:** 2026-05-02
**Source design:** `docs/plans/2026-05-02-kosmonotes-design.md`
...
1. Launch KosmoNotes from `Applications/`. Menu-bar icon visible. Popover opens on click.
...
| Bundle identifier (locks Keychain service) | **`dev.kosmonotes.studio`** | Locked Day 1 of Phase 0. Display name ("KosmoNotes" working title) is mutable post-ship; bundle ID is **not** — Keychain entries are tied to it |
...
- `App/KosmoNotesApp.swift` — `@main` with `MenuBarExtra("KosmoNotes", systemImage: "waveform.circle")` + empty popover (just a "Record" placeholder button)
...
- `IOPMAssertionCreateWithName` with `kIOPMAssertPreventUserIdleSystemSleep` reason "KosmoNotes recording in progress" — held for the lifetime of an active `RecorderSession`
...
| `docs/plans/2026-05-02-kosmonotes-design.md` | Deferred item updated; D13 + D14 added to Decision Log §15. |
